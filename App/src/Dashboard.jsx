...
const SectionHeader = ({ title, linkTo, accentColor }) => (
  <div className="flex justify-between items-center mb-4">
    ...
    <Link to={linkTo} className="text-sm font-medium flex items-center" style={{ color: accentColor }}>
      View all
...
const EmptyState = ({ message, linkTo, linkText, accentColor }) => (
  <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 text-center">
    ...
    <Link to={linkTo} className="mt-2 inline-flex items-center text-sm font-medium" style={{ color: accentColor }}>
      {linkText}
      <svg className="w-4 h-4 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
function Dashboard() {
  ...
  useEffect(() => {
    try {
      ...
      if (savedNotes) {
        ...
        const sortedPinnedNotes = [...pinned].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
  ...
  useEffect(() => {
    ...
    const overdue = savedTasks.filter((task) => !task.completed && task.dueDate && new Date(task.dueDate) < now).length;
    const highPriority = savedTasks.filter((task) => !task.completed && task.priority === "high").length;
    const completedProjects = savedProjects.filter((project) => project.completed).length;
    ...
    try {
      ...
      if (userData.name) {
        ...
        setUserName(userData.username.includes("@") ? userData.username.split("@")[0] : userData.username);
      }
    } catch (e) {
  ...
  const getDayTasks = (date) => {
    ...
    return tasks.filter((task) => task.dueDate && task.dueDate.startsWith(dateString));
  };
  ...
  const tasksToday = tasks.filter((task) => !task.completed && task.dueDate && task.dueDate.startsWith(today));
  
  // Calculate productivity score
  const productivityScore = Math.min(
    100,
    Math.round((stats.completedTasks * 10 + stats.completedProjects * 20) / Math.max(1, stats.totalTasks + stats.totalProjects))
  );
  ...
  return (
    <div className="container mx-auto px-4 pb-16">
      ...
      <AIAssistant isOpen={isAssistantOpen} onClose={() => setIsAssistantOpen(false)} accentColor={accentColor} />
      ...
      >
        <div className="flex flex-col md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="text-gray-600 dark:text-gray-400">
              You have <span className="font-medium">{tasksToday.length} tasks</span> scheduled for today
              {stats.overdueTasks > 0 && (
                <span> and <span className="font-medium text-red-600 dark:text-red-400">{stats.overdueTasks} overdue</span> tasks</span>
              )}
            </p>
          </div>
          <div className="mt-4 md:mt-0">
            <div className="flex items-center">
              <div className="mr-3">
                ...
                <p className="text-2xl font-bold" style={{ color: accentColor }}>{productivityScore}%</p>
              </div>
              <div className="relative w-16 h-16">
                <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100">
                  ...
                  />
                  <circle
                    style={{ color: accentColor, transition: "stroke-dashoffset 1s" }}
                    strokeWidth="10"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <motion.div
          ...
          className="col-span-1"
        >
          <SectionHeader title="Today's Tasks" linkTo="/tasks" accentColor={accentColor} />
        ...
        <motion.div
          ...
          className="col-span-1"
        >
          <SectionHeader title="Pinned Notes" linkTo="/notes" accentColor={accentColor} />
          ...
          {pinnedNotes.length > 0 ? (
            <div className="space-y-4">
              {pinnedNotes.map((note) => (
                ...
                >
                  <p className="text-sm font-medium mb-2 line-clamp-3">{note.text}</p>
      ...
      <motion.div
        ...
        className="mt-8"
      >
        <SectionHeader title="Recent Projects" linkTo="/projects" accentColor={accentColor} />
